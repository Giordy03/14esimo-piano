...
public class GameManager2P : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        {
            player1 = PhotonNetwork.Instantiate(playerPrefabName, new Vector3(0, -8, 0),  Quaternion.Euler(0, 0, 90));
            player1NameText.text = player1.GetComponent<PhotonView>().Owner.NickName;
        }
        else if (PhotonNetwork.CurrentRoom.PlayerCount == 2)
        {
            player2 = PhotonNetwork.Instantiate(playerPrefabName, new Vector3(0, 9, 0),  Quaternion.Euler(0, 0, 90));
    ...
    void UpdateScore(int player1Score, int player2Score)
    {
        ...
        if (player1 != null)
            player1.transform.position = new Vector3(0, -8, 0);
        
        if (player2 != null)
            player2.transform.position = new Vector3(0, 9, 0);
    }
    ...
    {
        Debug.Log("OnDisconnected callback triggered with reason: " + cause);
        SceneManager.LoadScene("SceneMainMenu");
    }
