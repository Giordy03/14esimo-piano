...
public class PlayerController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    void Start()
    {
       ...
       playerColor = GameManager2P.instance.colors[PlayerPrefs.GetInt("color", 0)];
        ...
        middleX = mainCamera.ScreenToWorldPoint(screenMiddle).x;
    }
    ...
    void Update()
    {
        if (view.IsMine)
        {
            if (Input.GetMouseButton(0))
            {
                ...
                if (mousePosition.x >= middleX && transform.position.x <= maxX)
                {
                    transform.Translate(Vector3.down * speed * Time.deltaTime);
                }
                else if (mousePosition.x < middleX && transform.position.x >= minX)
                {
